...
- Each row on the `/settings` user list has a "Send welcome email" button (mail icon) that emails the user a branded intro with their role and a short guide to the app's main pages, plus a login link (`APP_BASE_URL`). It's sendable on demand — there's no automatic send on user creation. Requires SMTP env vars (`SMTP_HOST`/`SMTP_USER`/`SMTP_PASS`, see `.env.example`); if unset, the button surfaces a clear "Failed to send" error instead of failing silently. Templates live in `apps/api/src/emails/welcomeEmail.ts`, sending goes through `apps/api/src/lib/mailer.ts` (nodemailer).
