...
- Non-admin users (Manager or Viewer) can optionally be restricted to specific projects — this is opt-in per user (most stay unrestricted, seeing everything as before), toggled via "Restrict to specific projects" on their Settings form, with a checklist of every project grouped by customer. A restricted user's entire view is filtered consistently: Customers/Employees only show records related to their granted projects, Planning/Reports/Gantt/Dashboard/Excel-PDF exports all scope down to those projects too, and attempting to edit or open a non-granted project directly (by ID) 403s/404s even though the role itself (e.g. Manager) would otherwise allow it. Admins are never restrictable — the option only shows for Manager/Viewer. The scoping logic lives in `apps/api/src/lib/accessScope.ts` (`getAccessScope`, `getAccessible*Ids`, `requireProjectAccess`); a restricted Manager who creates a new project is auto-granted access to it, since they'd otherwise lose visibility into what they just made.
- `requireAuth` re-fetches the user's role/isRestricted/isActive fresh from the DB on every request instead of trusting the (up to 7-day-old) JWT — so an admin toggling someone's role, restriction, or active status takes effect on their very next request, not after they happen to log out and back in.
- **Account setup is invite-based**: when an admin creates a user, the password field is optional — leaving it blank stores an unusable random hash and emails the user a branded **"Set your password"** invite (a one-time, 7-day link) so they choose their own credentials; the admin never has to convey a password out-of-band. Each `/settings` row also has a mail-icon button to (re)send that invite. A self-service **"Forgot password?"** link on the login page issues a short-lived (1-hour) reset link through the same mechanism. Tokens are single-use and stored only as SHA-256 hashes (`PasswordSetToken`); the raw token lives only in the emailed link. Public endpoints: `POST /auth/forgot-password` (always returns ok — no account enumeration), `GET /auth/set-password/:token` (validate), `POST /auth/set-password`. Set-password/forgot pages are public routes (`/set-password`, `/forgot-password`). Token logic is in `apps/api/src/lib/passwordTokens.ts`, emails in `apps/api/src/emails/accountEmails.ts`, sent via `apps/api/src/lib/mailer.ts` (nodemailer over any SMTP provider — see `.env.example`). If SMTP isn't configured the account is still created; the admin can resend the invite once mail works, or set a password directly by editing the user.
...
- Every page's content (not just the sidebar/login shell) is now routed through the `t()` translation helper in `apps/web/src/lib/i18n.tsx` — card titles, table headers, form labels, buttons, empty/error states, and status/role/employment/billing-type badges all switch between Hebrew and English. `t()` supports `{param}` interpolation (e.g. `t('planning.months', { n })`) and accepts loose string keys (`t(\`status.${status}\`)`) for values keyed off API enums, so badges built from dynamic data translate without a giant switch statement. Dashboard's "at risk"/"license attention" reasons are now returned by the API as a `reasonKey` (+ optional `reasonParams`) instead of a pre-formatted English sentence, so they translate too. The embedded Gantt chart (`gantt-task-react`) gets a `locale` prop so its calendar header and date columns follow the app language, but its static "Name/From/To" column header labels are the library's own chrome and stay in English (same known LTR-only limitation as before).
- **Actual vs. planned hours** are fed by uploading the attendance software's "customer monthly hours" export on the `/actuals` page (Admin only). The parser (`apps/api/src/services/actualsParse.service.ts`, via SheetJS which reads the legacy `.xls`/BIFF format `exceljs` can't) reads columns C=hours, G=date, H=employee, I=sub-project, K=project, L=customer, skips the trailing "total" row, and aggregates to (project, employee, month, sub-project). Uploaded files land as a `pending_review` batch; an admin reconciles any unmatched Hebrew names against app records in a preview dialog. Matching (`importMatch.service.ts`) tries a saved alias first, then a normalized name (Hebrew niqqud stripped, "lastName firstName" and "firstName lastName" both indexed); confirmed matches are saved as `ImportAlias` rows so the same names auto-match next time. Committing writes `TimeEntryActual` rows (sub-project detail kept for drill-down) and rolls them up into `MonthlyAllocation.actualHours` — which was always read as "consumed/actual" across Burn, Profitability, Utilization, Dashboard and Gantt but never populated until now. Re-uploading the same customer+month **replaces** that month's actuals (the prior batch is marked `superseded`); deleting a batch recomputes the affected `actualHours` back down. The headline surface is the **Reports → Planned vs actual** tab (planned/actual/variance/variance% per project, Excel+PDF export). Note: SheetJS (`xlsx@0.18.5`) carries known npm-audit advisories with no upstream fix in that line — acceptable here since it only parses trusted, admin-uploaded internal files, but worth revisiting if that ever changes.
...
- The dashboard's "Projects at risk" only flags the "remaining hours below 10%" reason for projects that actually have `hoursPaid > 0` — a project with zero hours paid/allocated has an undefined ratio, not a 100%-consumed one, so it's excluded from that check (though it can still show up separately if it's past its end date and not completed).
- Every page's header (`PageHeader`) shows a back arrow that calls browser history back (`navigate(-1)`), except the Dashboard itself since there's nothing meaningful to go back to from the app's home screen. Pass `showBack={false}` to opt any other page out if needed.
